...
// Screen showing the movie still (backdrop/poster) — looks like a real screen.
...
function Scene({ rows, cols, rowIndex, colIndex, screenImage, rowColors }) {
    ...
    return (
        <>
            ...
            <pointLight position={[0, 6, -7]} intensity={50} color="#c084fc" distance={40} />
...
export default function SeatViewPreview({ rowIndex = 0, colIndex = 0, rows = 8, cols = 10, screenImage = null, rowColors = null }) {
    return (
        <Canvas dpr={[1, 1.5]} gl={{ antialias: true }} style={{ width: "100%", height: "100%" }}>
            ...
            <Suspense fallback={null}>
                <Scene rows={rows} cols={cols} rowIndex={rowIndex} colIndex={colIndex} screenImage={screenImage} rowColors={rowColors} />
            </Suspense>
        </Canvas>
    );
}
